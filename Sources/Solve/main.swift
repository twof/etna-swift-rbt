...
// ETNA solve runner (product name `rbt`).
//   rbt <strategy> <property> [duration_seconds]
// The mutant under test is whichever marauder variant is active in the compiled
// `RBT` module — ETNA's driver activates it (source-swap) and rebuilds before
...
guard args.count >= 3 else {
    FileHandle.standardError.write(Data("usage: rbt <strategy> <property> [duration_seconds]\n".utf8))
    FileHandle.standardError.write(Data("properties: \(rbtProperties.joined(separator: ", "))\n".utf8))
    exit(2)
}
