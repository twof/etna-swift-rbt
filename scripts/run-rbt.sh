#!/bin/bash
# Run the built `rbt` solve binary with the patched toolchain's runtime on the
# dylib path. ETNA's `solve` capability invokes this.
#   run-rbt.sh <strategy> <property> [duration_seconds]
...
CONFIG="${CONFIG:-debug}"
exec env DYLD_LIBRARY_PATH="$RT" "$ROOT/.build/$CONFIG/rbt" "$@"
